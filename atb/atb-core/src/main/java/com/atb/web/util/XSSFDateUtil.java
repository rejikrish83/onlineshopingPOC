...
public class XSSFDateUtil
{
    ...
    private static final double CAL_1900_ABSOLUTE =
        (double) absoluteDay(new GregorianCalendar(1900, Calendar.JANUARY, 1)) - 2.0;

    /**
     * Given a Date, converts it into a double representing its internal Excel representation, which
     * is the number of days since 1/1/1900. Fractional days represent hours, minutes, and seconds.
     * 
     * @return Excel representation of Date (-1 if error - test for error by checking for less than
     *         0.1)
     * @param date the Date
     */
    ...
    public static double getExcelDate(Date date)
    {
        ...
        else
        {
            ...
            double fraction =
                (((calStart.get(Calendar.HOUR_OF_DAY) * 60 + calStart.get(Calendar.MINUTE)) * 60
     ...
     * 
     * NOTE: If the default <code>TimeZone</code> in Java uses Daylight Saving Time then the
     * conversion back to an Excel date may not give the same value, that is the comparison
     * <CODE>excelDate == getExcelDate(getJavaDate(excelDate,false))</CODE> is not always true. For
     * example if default timezone is <code>Europe/Copenhagen</code>, on 2004-03-28 the minute after
     * 01:59 CET is 03:00 CEST, if the excel date represents a time between 02:00 and 03:00 then it
     * is converted to past 03:00 summer time
     * 
     * @param date The Excel date.
    ...
    /**
     * given a format ID this will check whether the format represents an internal date format or
     * not.
     */
    ...
    public static boolean isCellDateFormatted(XSSFCell cell)
    {
        ...
        if (XSSFDateUtil.isValidExcelDate(d))
        {
